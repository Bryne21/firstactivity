...
<head>
    <meta charset="UTF-8">
    <title>Payroll System - Full Deductions</title>
    <style>
        body { margin: 0; font-family: 'Segoe UI', sans-serif; background: #f0f2f5; padding: 20px; }
        .card { background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); max-width: 1200px; margin: auto; }
        ...
        .custom-select { padding: 8px; border-radius: 4px; border: 1px solid #1a73e8; }
        ...
        .max-indicator { display: block; font-size: 10px; color: #ffeb3b; }
        input[type="number"] { width: 70px; padding: 5px; border: 1px solid #ddd; border-radius: 4px; text-align: center; }
        .deduction-input { background: #fff5f5; border: none !important; width: 60px !important; }
                ...
                <tr>
                    ...
                    <td>
                        <input type="number" name="days[]" id="days_<?= $id; ?>" 
                               value="0" step="0.5" min="0" max="<?= $max_weekdays ?>" 
                               oninput="calculatePayroll(this, <?= $id ?>)">
                    </td>
                    <td><input type="number" name="sss[]" id="sss_<?= $id; ?>" class="deduction-input" readonly value="0.00"></td>
                    <td><input type="number" name="philhealth[]" id="philhealth_<?= $id; ?>" class="deduction-input" readonly value="0.00"></td>
                    <td><input type="number" name="pagibig[]" id="pagibig_<?= $id; ?>" class="deduction-input" readonly value="0.00"></td>
...
function calculatePayroll(input, id) {
    ...
    if (days > maxVal) {
        alert("Limit reached: " + maxVal + " days.");
        input.value = maxVal;
        days = maxVal;
    }
